    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                ...
                urls = [url]
                urls += [urljoin(BASE_URL, url.attributes.get('href')) for url in urls_node if 'href' in url.attributes]
...
async def contient_attele_ou_monte(url: str, session: aiohttp.ClientSession) -> bool:
    try:
        ...
        if info_course:
            ...
            print(f"Span avec la classe 'infoCourse' non trouvé dans l'url: {url}")
            return False
